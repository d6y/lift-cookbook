...
Be aware that 404 pages, when rendered this way, won't have a location in the site map. That's because we've not included the _404.html_ file in the site map, and we don't have to because we're rendering via `NotFoundAsTemplate` rather than sending a redirect to _/404.html_. However, this means that if you display an error page using a template that contains `Menu.builder` or similar (as `templates-hidden/default.html` does), you'll see "No Navigation Defined".  In that case, you'll probably want to use a different template on your 404 page.
