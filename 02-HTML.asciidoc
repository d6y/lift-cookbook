...
Here's an example were we test out a CSS selector which adds a href attribute to a link:
...
Use `MockWeb.testReq` from Lift's TestKit with Specs2.

Add the dependency to your project in `build.sbt`:
...
To illustrate this, we will test a function that detects a Google crawler. Google identifies
...
We have the list of magic `botNames` that Google sends as a user agent, and we define a check, `known_?` that takes the user agent string, and looks to see if any robot satisfies the condition of being contained in the user agent string.

The `googlebot_?` method is given a Lift `Req` object and from this we look up the header.  This evaluates to a `Box[String]` as it's possible there is no header.  So we see if there exists in the `Box` a value that satisfies the `known_?` condition.

To test this, we create some a user agent string, prepare a `MockHttpServletRequest` with the header, and use Lift's `MockWeb` to turn the low-level request into a Lift `Req` for us to test with:
...
* `NodeSeq` wil; use "text/xml" (or you can supply an alternative).
* `String` uses "text/plain" (unless you supply an alternative).
* `Array[Byte]` does not set the content type.

In the example test above it would be tedious to have to set up the same code repeatedly for different user agents.  Specs2's Data Table provides a way to produce a compact to run different example values through the same test:
...
out the various scenarios we want to test against, and pipe them through a function.

The symbols used (`^`, `||`, `!!`, `|>`) may seem quirky, but they do help minimize the test boilerplate code around the important stuff: the examples under test.
...
I've put the expected value first, but there's no requirement to do that.
