...
For an introduction to Lift's Ajax and Comet features, see http://simply.liftweb.net[_Simply Lift_], Chapter 9 of _Lift in Action_ (Perrett, 2012, Manning Publications, Co.), or watch http://bit.ly/12unDnJ[Diego Medina's video presentation].
...
happen on the server and update the browser with the result. The pass:[<span class='keep-together'>interface is:</span>]
...
components pass:[<span class='keep-together'>dynamically.</span>]

See Also
^^^^^^^^

http://bit.ly/198KMMS[The source for `FocusOnLoad`] is worth checking out to understand how it and related commands are constructed.  This may help you package your own JavaScript functionality up into commands that can be used in CSS binding pass:[<span class='keep-together'>expressions.</span>]
...
.Factory
****
The `noCometSessionCmd.default.set` call is making use of Lift's http://bit.ly/16nYJVY[dependency injection]. Specifically, it's setting up the supply side of the dependency. Although we're setting a default here, it's possible in Lift to supply different behaviours with different scopes: request or session.
****
...
Being able to debug HTTP traffic is a useful way to understand how your Comet or Ajax application is performing.  There are many plugins and products to help with this, such as the http://mzl.la/14BkhyC[_HttpFox_ plugin for Firefox].
...
Add Sebastian Tschan's http://bit.ly/1ac2yAj[_jQuery File Upload_ widget] to your project, and implement a REST end point to receive files.
...
In the REST service, the uploaded files are available via the `uploadedFiles` method on the request. When Lift receives a multipart form, it automatically extracts files as pass:[<span class='keep-together'><literal>uploadedFiles</literal>,</span>] each of which is a `FileParamHolder` that gives us access to the `fileName`, `length`, `mimeType`, and `fileStream`.
...
Diego Medina has posted a http://bit.ly/lift-restupload[Gist of Lift REST code] to integrate more fully with the image upload and image reviewing features of the widget, specifically implementing the JSON response that the widget expects for that pass:[<span class='keep-together'>functionality.</span>]
...
this would mean the `NodeSeq` passed to us would just be the text node representing "?".  With a richer template we can use CSS selectors. For example, we can modify the pass:[<span class='keep-together'>template:</span>]
