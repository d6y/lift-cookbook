...
http://bit.ly/198KMMS[The source for `FocusOnLoad`] is worth checking out to understand how it and related commands are constructed.  This may help you package your own JavaScript functionality up into commands that can be used in CSS binding pass:[<phrase role='keep-together'>expressions.</phrase>]
...
Diego Medina has posted a http://bit.ly/lift-restupload[Gist of Lift REST code] to integrate more fully with the image upload and image reviewing features of the widget, specifically implementing the JSON response that the widget expects for that pass:[<phrase role='keep-together'>functionality.</phrase>]
