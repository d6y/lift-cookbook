...
The JavaScript that performs the focus simply looks up the node in the DOM by ID and calls `focus` on it. Although the previous example code doesn't specify an ID, the `FocusOn` command is smart enough to add one automatically for us.
