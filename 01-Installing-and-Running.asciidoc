...
Tutorials for Lift can be found in http://simply.liftweb.net/[_Simply Lift_], Instant Lift Web Applications How-to (PACKT Publishing), and in _Lift in Action_ (Manning Publications Co).
...
Triggered execution has a number of settings you can adjust, as described in http://www.scala-sbt.org/release/docs/Detailed-Topics/Triggered-Execution[the SBT documentation].

See http://www.scala-sbt.org/release/docs/Detailed-Topics/Command-Line-Reference[the SBT Command Line Reference] for an overview of available commands.
...
. Start the server from a Terminal window using the JAR file you just found: `java pass:[<phrase role='keep-together'><literal>-cp</literal></phrase>] /path/to/h2-version.jar org.h2.tools.Server`
