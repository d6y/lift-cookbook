...
downloaded a ZIP containing four bare-bones Lift applications, and then started 
one of them via the build too.
...
libraries required by this Lift application. 
...
The `container:start` command is starting Jetty.

If you take a look at `build.sbt` you'll see:
