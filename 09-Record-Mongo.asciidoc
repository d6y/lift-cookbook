...
You want to connect to a MongoDB database.(((MongoDB, connecting to)))
...
The full URL scheme for MongoDB is more complicated, allowing for multiple hosts and connection parameters, but the previous code handles optional username and password fields and may be enough to get you up and running with your MongoDB pass:[<span class='keep-together'>configuration.</span>]
...
There's a http://bit.ly/lift-mongomap[discussion on the mailing list] regarding the limited type support in `MongoMapField` and a possible way around it by overriding pass:[<span class='keep-together'><literal>asDBObject</literal></span>].
...
http://bit.ly/144UdJO[The Lift wiki] describes `BsonRecord` in more detail.
...
The motivation for Rogue is described in a http://bit.ly/120WkLF[Foursquare engineering blog post].
...
As an example, if we tried to insert a duplicate city, our call to `save(true)` would pass:[<span class='keep-together'>result in:</span>]
...
Running everything in `Boot` may be a little heavy handed, especially if you are starting up various services and background tasks.  All we need to do is define a database pass:[<span class='keep-together'>connection.</span>] For example, using the sample code presented in <<ConnectingToMongo>>, we could initialise a conection with:
...
The changes in the 2.x series of Specs2 are 
documented on http://bit.ly/specs2-nterpolated[Eric Torreborre's blog].
=====================================================================
...
`test-only` together with modifications to `around` in `MongoTestKit` can be a good way to track down any issues you have with a test.  By disabling the call to `destroyDb()`, you can jump into the MongoDB shell and examine the state of the database after a test pass:[<span class='keep-together'>has run.</span>]
...
http://bit.ly/11NldyA[The Lift MongoDB Record library] includes a variation on testing with Specs2, using just `Before` and `After` rather than the `around` example used in this recipe.
...
http://bit.ly/144UxZk[The Lift wiki] describes more about unit testing and Lift sessions.(((range="endofrange", startref="ix_DBmdbr")))(((range="endofrange", startref="ix_MDBunit")))(((range="endofrange", startref="ix_UTestspec")))
