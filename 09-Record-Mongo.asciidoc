...
The query language itself is expressive, and the best place to explore the variety of queries is in the `QueryTest` specification in the source for Rouge.  You'll find a link to this in the README of the project on Github.
...
<<ConnectingToMongo>> for connecting to MongoDB and <<QueryingWithRouge>> for querying with Rouge.
