...
You can request that Lift show a different page by setting `LiftRules.siteMapFailRedirectLocation` in _Boot.scala_:
...
By evaluating to `Empty` for other requests (i.e., https requets), Lift will continue passing the request
