...
When a request reaches Lift there are a number of points where you can jump in an control what Lift does,  send back a different kind of response, or control access.  This chapter looks at the pipeline through examples of different kinds of `LiftResponse` and configurations.
...
...we are making use of the `write(byte[])` method on Java's `OutputStream` (`out`), and sending it
