...
The way Typeahead works is that we attach it to our input field, and tell it to call a JavaScript function when it need to make suggestions.  That JavaScript function is going to be called `askServer`, and it is given two arguments: the input the user has typed so far (`query`), and a JavaScript function to call when the suggestions are available (`callback`).  The Lift snippet needs to use the `query` value and then call the JavaScript `callback` function with whatever suggestions are made.((("autocomplete suggestions", "query/callback functions")))
...
them to select multiple values.((("forms processing", "multiple selections", id="ix_FPmulval", range="startofrange")))(((multiple selections)))(((select boxes)))(((checkboxes, multiple options)))
...
The issue relating to no options being selected is on https://github.com/lift/framework/issues/1139[GitHub].(((range="endofrange", startref="ix_FPmulval")))
