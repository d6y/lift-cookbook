...
to completely control how the form is rendered (instead of just as a linear
dump of fields).

Solution
^^^^^^^^

Use `CssBoundLiftScreen`, field bindings for naming placements, and optionally
use field bindings to override the layout of specific individual field
elements. Requires Lift 2.5+.
...
object AccountInfoEditor extends CssBoundLiftScreen {
  ...
  val firstName = field("First Name", accountToEdit.first_name, trim, valMinLen(1, "First name is required"), FieldBinding("first_name"))
  val lastName = field("Last Name", accountToEdit.last_name, trim, valMinLen(1, "Last name is required"), FieldBinding("last_name"))
  val address = field("Address", accountToEdit.address, trim, valMinLen(1, "Address is required"), FieldBinding("address", Self))
  val city = field("City", accountToEdit.city, trim, valMinLen(1, "City is required"), FieldBinding("city"))
  val state = field("State", accountToEdit.state, trim, valMinLen(1, "State is required"), FieldBinding("state"))
  ...
}
---------------------------------------------------------
...
<div data-lift="AccountInfoEditor">
  <div>
      <!-- Drop elements that are needed by general lift screen where you want them...e.g. the Next button at the top -->
    <button class="next"></button>
  </div>
  <div class="fields">
    <h2>Basic Info</h2>
    <!-- You can set classes on these if you like...they are not used by the rendering code -->
    <div class="younameit" id="accountedit_first_name_field">
      <!-- The template used for laying out the fields is in accounts/account_edit_field.html -->
    </div>
    <div id="accountedit_first_name_field"></div>
    <div id="accountedit_last_name_field"></div>
    <!-- The code binds this field with Self, meaning the field template is inline -->
    ...
    <div id="accountedit_city_field"></div>
    <div id="accountedit_state_field"></div>
</div>
---------------------------------------------------------
...
`CssBoundLiftScreen` uses the same model (by default) as LiftScreen, but uses
...
`CssBoundLiftScreen` resolves this by allowing you to optionally control every
element of the form rendering, from form field placement to the layout of a
single field. You do so by embedding the template at the snippet usage site.
...
template file in accounts/account_edit_field.html).

The rest is very easy. You must supply a formName declaration (in Scala) for
`CssBoundLiftScreen`, and FieldBindings as an argument to each field. This,
combined with an internal function (that you can override) will generate unique
(but known) names that can be used in the template.

So, if you name your form "myform", and bind a field to "address", then your
HTML template should include a div with the ID "myform_address_field".  For
normal bindings (which use your field template), that is all you need.  

If you also need to tweak the field layout for a particular field, you may
specify `Self` in the Scala field binding, which indicates the template for
that particular field will be nested within the div with the field's ID.
...
[[AdvancedEditorForLiftScreen]]
Extending LiftScreen with a Javascript HTML editor
~~~~~~~~~~~

Problem
^^^^^^^

You want your users to be able to use an editor like TinyMCE or CKEditor
for a textarea, and you want to use LiftScreen or CssBoundLiftScreen.

Solution
^^^^^^^^

Create a subclass of CssBoundLiftScreen, and add a new field definition.
There are several tricks that will come in handy:

* The form can be re-rendered on the same page in AJAX mode, so you might need to remove the prior instance of the editor before re-installing it.  If you embed the screen in a light-box (like JQuery UI dialogs), you will have problems with timing. The solution shown builds in a 1 second embed delay.
* Most fancy editors don't store the edited content in the text area, so
you'll have to hack into the "finish" step so that you can copy the 
content of the editor back to the original lift-generated text area.
...
abstract class Screen extends CssBoundLiftScreen {
  val sendButtonID = randomString(20)

  protected def ckeditor(name: => String, defaultValue: => String, rows: Int, cols: Int, stuff: FilterOrValidate[String]*): Field { type ValueType = String } = {
    val id = nextFuncName
    val eAttr: List[SHtml.ElemAttr] = (("class" -> "hidden"): SHtml.ElemAttr) ::
      (("id" -> id): SHtml.ElemAttr) ::
      (("rows" -> rows.toString): SHtml.ElemAttr) ::
      (("cols" -> cols.toString): SHtml.ElemAttr) :: grabParams(stuff)

    // Embed the real textarea using Lift, and then emit javascript to
    // morph it into an advanced editor. The delay helps with things 
    // like forms embedded in animated dialogs.
    makeField[String, Nothing](name,
      defaultValue,
      field => {
        <span>
          { SHtml.textarea(field.is, field.set(_), eAttr: _*) }
          {
            Script(JsRaw("""setTimeout(function(){
                         var editor = CKEDITOR.instances['%s'];
  ...
  /**
   * Hack into the finish button to make sure we copy data out of ckeditor
   * instances into their respective real field.
   */
  override protected def renderAll(currentScreenNumber: Box[NodeSeq],
    screenCount: Box[NodeSeq], wizardTop: Box[Elem], screenTop: Box[Elem], fields: List[ScreenFieldInfo],
    prev: Box[Elem], cancel: Box[Elem], next: Box[Elem], finish: Box[Elem], screenBottom: Box[Elem],
    wizardBottom: Box[Elem], nextId: (String, () => JsCmd), prevId: Box[(String, () => JsCmd)], cancelId: (String, () => JsCmd),
    theScreen: AbstractScreen, ajax_? : Boolean): NodeSeq = {

    // grab the old value of the onclick generated for the finish button
    val old = (for (
      n <- finish;
      onclick <- n.attribute("onclick")
    ) yield onclick.toString)
    // Hack in the necessary extra steps
    val pullText = """jQuery('textarea').each(function () { try { var $textarea = $(this); $textarea.val(CKEDITOR.instances[$textarea.attr('id')].getData()); } catch(e) {} } );"""
    val hackedFinish = finish.map(n => n % ("onclick" -> (pullText + old.openOr(""))))

    super.renderAll(currentScreenNumber,
      screenCount,
      wizardTop,
      screenTop,
      fields,
...
then you can use your new screen type with declarative logic to get a fancy
editor:

[source,scala]
---------------------------------------------------------
class EmailScreen extends Screen {
   val to = field("Subject", "")
   val subject = field("To", "")
   val editor = ckeditor("Email Body", "", 20, 120)
}
...
The solution above embeds the popular CKEditor as a usable field type. If you
wish to use that field type in a wizard, you will also need to hack the 
editor extraction into the next/previous button actions.
