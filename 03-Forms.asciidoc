...
This would normally produce a regular text input field, but we can change that by adding JavaScript to attach the date picker to the input field.  You could do this in the template, but in this example we're attaching it as part of the snippet code:
...
As more browsers implement these types, it will become possible to depend on them.  However, you can default to the browser-native date pickers today and fall back to a JavaScript library as required.  The difference is show in <<DatePickerScreenGrabs>>.
...
image::images/date-picker.png[width=640]
...
When this snippet runs, and the page is rendered, the JQuery UI date picker will be attached to input fields of `type="date"` only if the browser doesn't support that type already.  An alternative approach would be to detect support via the browser's _user agent_ string, if you had a list of user agents that support this feature.
...
<<TestingReq>> includes a description of how to access the user agent string.



[[Autocomplete]]
Use an Autocomplete Widget
~~~~~~~~~~~~~~~~~~~~~~~~~~

Problem
^^^^^^^

You want to provide an autocomplete widget to provide users with suggestions as they type into a text field.

Solution
^^^^^^^^

Use the jQuery UI autocomplete widget, for example by using the `AutoComplete` class from the Lift Widgets module.
...
We want to connect the `AutoComplete` class to the element with the ID of "autocomplete":
...
The last line of this snippet shows the binding of the `AutoComplete` class, which takes a default value to show, a function which will produce suggestions based on the value of the text entered, and a function to call when the form is submitted.
...
image::images/autocomplete-widget.png[width=540]

When the form is submitted, the `submit` function will be passed the selected value, which is being logged:
...
The autocomplete widget uses jQuery autocomplete. This can bee seen by examining the `NodeSeq` produced by the `AutoComplete.apply` method:
...
// <![CDATA[
  jQuery(document).ready(function(){
    ...
    jQuery("#F846528841916S3QZ0V").autocomplete(data, {minChars:0,matchContains:true}).
...
What's happening here is that the jQuery UI autocomplete JavaScript and CSS that's bundled with the Lift Widgets module is being included on the page. Recall from <<AddToHead>> that Lift will merge the `<head>` part of this markup into the `<head>` of the final HTML page.

When the page loads, the jQuery UI `autocomplete` function is bound to the input field, and configured with a URL which will deliver the user's input to our `suggest` function.  All we need to do is return a `Seq[String]` for the jQuery autocomplete code to display.

Submitting New Values
+++++++++++++++++++++

One peculiarity with the AutoComplete widget is that if you type in a new value and press submit, it is not sent to the server.  That is, you need to click on one of the suggestions to select it.  If that's not the behaviour you want, you can adjust it.

Inside our `render` method we can take the value of the input field when the field loses focus, for example when the submit button is pressed:
...
With this in place a user can type in a language not on the list and it will be sent to the server.

Alternative Auto Complete JavaScript
++++++++++++++++++++++++++++++++++++

Looking at the way the widget module builds autocomplete functionality may give you an insight into how you can incorporate other JavaScript autocomplete libraries into Lift.  The idea is to include the JavaScript library, connect it to an element on the page, and then arrange for suggestions to trigger a call on the server.  Of course, if you only have a few items for the user to pick from, you could just include those items on the page, rather than making a round trip to the server.

As an example, we can look at the _Typeahead_ component that is included in Twitter Bootstrap.

In the template, the changes are to include the library and mark the input field for typeahead:
...
We've included a placeholder called "ID" for the JavaScript that will call back to the server.

The way Typeahead works is that we attach it to our autocomplete input field, and tell it to call a JavaScript function when we need to make a suggestion.  That JavaScript function is going to be called `askServer`, and it is given two arguments: the input the user has typed so far, and a JavaScript function to call with the results.  The Lift snippet needs to call that function with whatever suggestions we make.
...
Working from the bottom of the snippet, we bind a regular Lift `SHtml.text` input to the autocomplete field.  This will receive the selected value when the form is submitted.  We also bind the JavaScript placeholder to a function definition called `askServer`.  This is the function that Typeahead will call.

The function takes two arguments: the `query` and a function called `callback`. The body of `askServer` causes it to run something called `runSuggestion`, which is a `jsonCall` to the server, with the value of the `query`.

The suggestions are made by the `suggest` function which expects to be able to find a `String` in the passed-in value, which it uses to find matches in the list of `languages`.  These are returned as a `JArray` of `JString`, which is treated as JSON data back on the client.

What does the client do with the data? It calls the `callback` function with the suggestions, and they are displayed.
...
As the user types into the text field, Typeahead calls `askServer` with the input supplied, and Lift's Ajax support arranges for that value to be passed to our `suggest` function on the server, and for the results to be passed to `callback`. At that point, Typeahead takes over again, and displays the suggestions.
...
The behaviour the widget module with new values is described in a ticket on the module's GitHub page: https://github.com/liftmodules/widgets/issues/4[https://github.com/liftmodules/widgets/issues/4]. Enhancing modules is one route to get involved with Lift, and <<ContributingAndHelp>> describes other ways to contribute.
...
image::images/multiselect.png[width=214]
...
The template is unchanged, and the snippet has been modified to introduce a `current` value, a hidden function to reset the value, and we've bound to the submit button to simply log the selected values when the form is submitted.
...
The only changes to the code are to produce a `List[(Item,String)]`, use an `Item` as a default:
