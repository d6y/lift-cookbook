...
With the validation function defined (`valUnique`), we include it in the list of `validations` on the
`name` field:
...
Filters are run before validation. As an example, this means if you have a minimum length validation and the trim filter, users cannot pass the validation test by included spaces on the end of the value they enter.
...
This function is splitting the input string on spaces, converting each word into an array of characters, converting the first character into upper case, and then gluing the strings back together.
...
The database column backing the `UniqueIdField` in this recipe will be a `varchar(32) not null` or similar and will hold a value like "GOJFGQRLS5GVYGPH3L3HRNXTATG3RM5M".

As the value is made up of just letters and numbers, it makes it easy to use in URLs as there are no characters to escape. For example, it could be used in a link to allow a user to validate their account when sent the link over email, which is what `ProtoUser` uses it for.
...
If you only need create time, or updated time, just mix in the `Created[T]` or `Updaed[T]` trait instead of `CreatedUpdated[T]`.
...
The setLogger is documented at: http://squeryl.org/miscellaneous.html[http://squeryl.org/miscellaneous.html].
