...
In _Boot.scala_ call `initWithSquerylSession` with a `DataSource` looked up from the JDNI context:
...
In _Boot.scala_ we can print out the schema...
...
It can be inconvenient to have to enable logging in each snippet during development.  To trigger logging for all snippets, you can modify the `addAround` call in _Boot.scala_.
...
To enable logging in each session, we can modify _Boot.scala_ to mix in logging and enable logging on every session:
...
The `setCharacterEncoding("UTF-8")` call in _Boot.scala_ is being applied to every `HTTPRequest` which ultimately, in a servlet container, is applied to a `ServletRequest`. This is how parameters in a request are going to be interpreted by the servlet container when received.
