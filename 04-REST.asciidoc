...
The following code will call the service:

[source, objc]
---------------------------------------------------------
-(NSMutableURLRequest*)postMutableURLRequestFactory:(NSURL*)url
  fileNameWithExtension:(NSString*)fileNameWithExtension namesDict:(NSDictionary*)namesDict{
...
There are many ways to do HTTP POST from iOS. However, it can be confusing to identify the correct way that works, especially without the aid of external library. The example above uses the  iOS native API, NSMutableURLRequest and NSURLConnection.

Another uncommon way is to use AFNetworking, ian external library for iOS development. It is well-respected by the Stackoverflow community and can cope with many scenarios and simple to use. However, there is chance that it maybe deprecated at some point and this method is comparatively less flexible than the native methods.
...
The iOS native method is more versatile and gives a starting point to developers to craft their own solution such as making the Content-Type more specific. The current solution works posting media file under 4MB along with few text JSON parameters or purely text-based JSON parameters and is not intended to cover all scenarios.

Since AFNetworking is more of a standard and tested than the customized one and is designed to support many situations,

The current example relies on multi-form construction for a simple JSON HTTP Post that follow RFC1867:
...
see lines 178-188
http://main.scala-tools.org/mvnsites/liftweb-2.0/framework/scaladocs/net/liftweb/http/Req.scala.html#Some(296)
