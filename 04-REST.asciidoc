...
----------------------
curl -X POST --data-binary "@dog.jpg" -H 'Content-Type: image/jpg' http://127.0.0.1:8080/upload
<?xml version="1.0" encoding="UTF-8"?>
...
Use the JSON domain specific language (DSL). For example:
...
Create `sitemap.html` in your `webapp` folder with
...
You may be wondering why we've used REST here, when we could have used a regular HTML template and snippet. The reason is that we want XML rather than HTML output.  We use the same mechanism, but invoke it and wrap it in an `XmlResponse`.  The `S.render` method takes a `NodeSeq` and a `HTTPRequst`. The first we supply by running the `sitemap.html` snippet; the second is simply the current request.  `XmlResponse` requires a `Node` rather than a `NodeSeq`, which is why we call `head`--as there's only one node in the response, this does what we need.

Note that Google Sitemaps need dates to be in ISO 8601 format. The built-in `java.text.SimpleDateFormat` does not support this format prior to Java 7. If you are using Java 6 you need to use `org.joda.time.DateTime` as we are in this example.  With Java 7 "yyyy-MM-dd'T'HH:mmXXX" can be used as the pattern for the formatting.
...
One common and easiest way to do this is via AFNetworking. And another uncommon way is to use iOS native API, NSMutableURLRequest and NSURLConnection.

A short example:

[source,scala]
---------------------------------------------------------
 serve ("welcome"/ "cookbook" prefix {
    case  Nil Post req => {
      val returnValue: Box[JValue] =  for {
           value <- req.param("key")
       } yield {
             println(value)
             // do something and return JValue e.g. (("command"->"somevalue") ~("message"->("true"):JValue)
          }
      val jVar:JValue =  returnValue.openOr(  ("command"->"session") ~("message"->"false"): JValue)
      jVar
    }
---------------------------------------------------------
...
[source, objc]
---------------------------------------------------------
-(NSMutableURLRequest*)postMutableURLRequestFactory:(NSURL*)url fileNameWithExtension:(NSString*)fileNameWithExtension namesDict:(NSDictionary*)namesDict{

    NSMutableURLRequest *request= [[NSMutableURLRequest alloc] init] ;
    [request setURL:url];
    [request setHTTPMethod:@"POST"];

    NSData* jsonData = [NSJSONSerialization dataWithJSONObject:namesDict options:0 error:nil];

    [request addValue:[NSString stringWithFormat:@"%i",[jsonData length] ] forHTTPHeaderField: @"Content-Length"];
    ...
    __block NSInteger counter = [namesDict count];
    [namesDict enumerateKeysAndObjectsUsingBlock:^(NSString* key, NSString *object, BOOL *shouldStop){
        [postbody appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n\r\n",key] dataUsingEncoding:NSASCIIStringEncoding]];
            [postbody appendData:[[NSString stringWithFormat:@"%@",object] dataUsingEncoding:NSASCIIStringEncoding]];
...
//Usage
     ...
     mutableURLRequest =  [self postMutableURLRequestFactory:[NSURL URLWithString:urlString] fileNameWithExtension:@"" namesDict:[NSDictionary dictionaryWithObjectsAndKeys:@"someValue", @"key", nil]]];

     NSURLConnection *con = [[NSURLConnection alloc] initWithRequest:mutableURLRequest  delegate:self];
---------------------------------------------------------
...
//AFNetworking example
  ...
  NSMutableDictionary* params =[NSMutableDictionary dictionaryWithObjectsAndKeys: @"somevalue", @"key", nil];

     NSMutableURLRequest *request =  [self multipartFormRequestWithMethod:@"POST" path: "http://www.example.com/welcome/cookbook" ] parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
  ...
  //completion block and do something after the request has been completed.
  }
...
There are many ways to do HTTP POST from iOS. However, it can be confusing to identify the correct way that works, especially without the aid of external library. This example also supports JSON post as illustrated as key and value in this example.


The current example relies on multi-form construction for a simple JSON HTTP Post. This may benefit a JSON POST with  media data. If no media data on the POST, probably there is a way to do this without using multi-form.

See Also
^^^^^^^^

http://simply.liftweb.net/index-5.4.html#prev[http://simply.liftweb.net/index-5.4.html#prev].
