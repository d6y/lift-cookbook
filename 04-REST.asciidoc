...
[source, objc]
---------------------------------------------------------
static NSString *url = @"http://localhost:8000/shout";
...
The iOS native method is more versatile and gives you starting point to craft your own solution, such as by making the Content-Type more specific. However, AFNetworking is popular and you may prefer that route.
