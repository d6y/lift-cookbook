...
This chapter looks at recipes around REST web services, via Lift's `RestHelper` trait. For an introduction, take a look at the https://www.assembla.com/spaces/liftweb/wiki/REST_Web_Services[Lift wiki page] and http://simply.liftweb.net[chapter 5 of _Simply Lift_].((("REST web services", id="ix_REST", range="startofrange")))(((RestHelper)))
...
You'll find an explanation of the Sitemap Protocol on Google's http://support.google.com/webmasters/bin/answer.py?hl=en&answer=156184[Webmaster Tools site].
...
You want to make an HTTP POST from a native iOS device to a Lift REST service.((("REST web services", "native iOS applications and")))(((HTTP POSTs)))((("iOS applications, HTTP POSTs from")))
