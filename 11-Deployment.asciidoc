...
Because the JDNI setting is only defined in _cloudbees-web.xml_ it will
...
To get the public hostname, you need to make a HTTP request to _http://instance-data/latest/meta-data/public-hostname_, as documented at https://developer.cloudbees.com/bin/view/Main/Finding+out+app+port+and+hostname[https://developer.cloudbees.com/bin/view/Main/Finding+out+app+port+and+hostname].  For example:
...
This URL contains a user name, password, host and database name, but needs to be manipulated to be used by JDBC.  To do so, you might include the following in _Boot.scala_:
...
For it to run _build.sbt_ needs the appropriate dependencies for Record and PostgresSQL:
