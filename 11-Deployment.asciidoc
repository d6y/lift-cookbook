...
Beanstalks enables _auto scaling_ by default. That is, it launches a single instance of your Lift application, but if the load increases above a threshold, up to four instances may be running.(((auto scaling)))((("Lift applications", "autoscaling of")))(((Lift Web Framework, stateful features of)))(((session affinity)))((("browsers", "session affinity")))(((sticky sessions)))(((load balancers)))
...
Note, though, that the application was stopped with a _SIGTERM_.  This is a Unix signal sent to a process, the JVM in this case, to request it to stop.  Unfortunately, the Tomcat application on Heroku does not use this signal to request Lift to shutdown. This may be of little consequence to you, but if you do have external resources you want to release to other actions to take at shutdown, you need to register a shutdown hook with the JVM.(((SIGTERM)))(((hooks)))(((shutdown hooks)))
...
There are a variety of technologies you can use to accomplish this, such as databases, message systems, actor systems. For this recipe we will use the RabbitMQ message service, but there are examples using CouchDB and Amazon's Simple Notification Service in the "See Also" section.(((RabbitMQ message service)))(((CouchDB)))((("Amazon's Simple Notification Service (SNS)")))
