...
This example `display` function is long but mostly straightforward. The `body` value handles multipart messages by extracting each body part.  This is triggered when sending more structured emails, such as the HTML emails described in <<HTMLEmail>>.
...
`ScheduledFuture` is documented via the Java Doc for `Future` at http://docs.oracle.com/javase/6/docs/api/java/util/concurrent/Future.html[http://docs.oracle.com/javase/6/docs/api/java/util/concurrent/Future.html].  If you're building complex, low-level, cancellable concurrency functions, it's advisable to have a copy of _Java Concurrency in Practice_ close by (written by Goetz, _et al._, Addison-Wesley Professional).
