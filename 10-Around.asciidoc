...
What if the request doesn't return a 200? In that case, with the code we have, we'd get an exception such as: "Unexpected response status: 404." There are a few ways to change that.((("404 (unexpected response) status")))((("404 (unexpected response) status", sortas="fourzerofour")))((("unexpected response (404) staus")))
