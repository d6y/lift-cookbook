...
However, some mail services such as SendGrid do require `mail.smtp.auth=true` to be set, and that should go into your Lift properties file or set as a JVM argument: pass:[<span class='keep-together'><literal>-Dmail.smtp.auth=true</literal></span>].(((SendGrid)))
...
Note that the result `country` is not a `String` but a `Promise[String]`, and we use pass:[<span class='keep-together'><literal>apply</literal></span>] to wait for the resulting value.
