...
This example `display` function is long mostly straight-forward. The `body` value handles multi-part messages by extracting the each body part.  This is triggered when sending more structured emails, such as the HTML emails described in <<HTMLEmail>>.
...
ensures the mime type of the email is _text/html_. Despite the name of
...
This message would be sent as a _multipart/alternative_:
...
To attach a pre-made file, you can use `LiftRules.loadResource` to fetch content from the classpath.  As an example, if our project contained a file called _Kepler-22b_System_Diagram.jpg_ in the _src/main/resources/_ folder, we could load and attach it like this:
