...
This example `display` function is long but mostly straightforward. The `body` value handles multipart messages by extracting each body part.  This is triggered when sending more structured emails, such as the HTML emails(((HTML email))) described in <<HTMLEmail>>.
