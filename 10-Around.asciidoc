...
Note that the result `country` is not a `String` but a `Promise[String]`, and we use pass:[<phrase role='keep-together'><literal>apply</literal></phrase>] to wait for the resulting value.
...
#REST[REST] explains how to use the `RestHelper`.

https://github.com/SparkySparkyBoom/ObeseRabbit[ObeseRabbit] is a small application showcasing this feature.

This recipe is based on this https://www.assembla.com/wiki/show/liftweb/StreamingResponse[article]
