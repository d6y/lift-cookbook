...
[quote, David Pollack, http://bit.ly/lift-expects[http://bit.ly/lift-expects]]
____
...
There's nothing special about modules: they are code, packaged and used like any other dependency. What makes them possible is the exposure of extension points via `LiftRules`. The main convention is to have an `init` method that Lift applications can use to initialise your module.
