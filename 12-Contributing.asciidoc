[[ContributingAndHelp]]
Contributing, Bug Reports, and pass:[<span class='keep-together'>Getting Help</span>]
----------------------------------------------------------------------------------------
...
You've found a bug and want to report it.((("bugs, reporting of")))(((reporting bugs)))((("questions about Lift, how to ask")))((("Lift Web Framework", "bug reports")))
...
You can find the template file http://bit.ly/19AllpY[on GitHub].
...
The source to this book is https://github.com/d6y/lift-cookbook/[on GitHub].
...
JavaScript to the head of every page.((("S.", "putInHead")))
...
However, we do not want to expose all files on the classpath to anyone visiting our application. To avoid that, Lift looks for resources inside a _toserve_ folder, which for our purposes means files and folders inside _src/main/resources/toserve_.  You can think of pass:[<span class='keep-together'><em>/classpath</em></span>] meaning _toserve_ (although, you can change those values via `LiftRules.resourceServerPath` and `ResourceServer.baseResourceLocation`).
...
To publish to Sonatype, take a look at http://bit.ly/163Hb0y[their guide]. CloudBees offers an http://www.cloudbees.com/foss/foss-dev.cb[open source repository].
